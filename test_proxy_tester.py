...
class ProxyTestHarness:
    ...
    def start_proxy(self, *extra_args):
        ...
        cmd = [
            ...
            "--port", str(self.expected_port),
            *extra_args,
            "--",
            "sleep", "3600"  # Long-running process to keep proxy alive
        ]
...
def test_error_dns_nonexistent(proxy):
    ...
    with pytest.raises((requests.exceptions.ProxyError, requests.exceptions.ReadTimeout)) as exc_info:
...
def test_proxy_startup(proxy):
    ...
    assert proxy.port == proxy.expected_port, f"Port mismatch: got {proxy.port}, expected {proxy.expected_port}"
...
def test_proxy_intercept(proxy):
    ...
    proxy.start_proxy(
        ...
        '--return-header', 'X-Test: Custom Header',
        '--return-data', '{"status": "intercepted"}'
    )
    proxy.setup_environment()
...
def test_intercept_headers(proxy):
    ...
    proxy.start_proxy(
        ...
        '--return-header', 'X-Custom-String: Hello, World!',
        '--return-header', 'X-Custom-Empty:',  # Empty value
        '--return-header', 'X-Custom-Special: Hello: world; something=value',  # Special chars
        '--return-header', 'X-Custom-Long: ' + 'x' * 1000,  # Long value
        '--return-header', 'Set-Cookie: cookie1=value1',  # First cookie
        '--return-header', 'Set-Cookie: cookie2=value2',  # Second cookie
...
def test_concurrent_connections(proxy):
    ...
    # Verify we got exactly 4 unique connections
    assert len(connections) == 4
    
    # Track which connection handled which request index
    handled_indices = {}
    
    # Verify each connection completed its flow
    for cid, lines in connections.items():
        assert any('[C<>P] SSL handshake completed' in line for line in lines)
        assert any('[P<>S] SSL handshake completed' in line for line in lines)
        
        # Look for request line and extract index
        request_line = next((line for line in lines if '?ndx=' in line), None)
        assert request_line is not None, f"Connection {cid} missing request line"
        ndx = int(request_line.split('?ndx=')[1].split(' ')[0])  # Extract index from query string
        print(f"Connection {cid} handled request {ndx}")
        
        # Track which connection handled which index
        handled_indices[ndx] = cid
        
        # Verify connection completed
        assert any('Connection closed' in line for line in lines)
    
    # Verify all indices were handled exactly once
    assert sorted(handled_indices.keys()) == [0, 1, 2, 3], "Not all indices were handled"
    print("\nIndex assignments:", handled_indices)
...
def test_proxy_delay(proxy):
    ...
    if enforcing_lines:
        ...
        if end_lines:
            ...
            assert end_time < delay + 0.1, f"Delay too long: {end_time:.3f}s >= {delay + 0.1}s"
        else:
...
def test_concurrent_connections(proxy):
    ...
    for cid, lines in connections.items():
        ...
        assert any('[C<>P] SSL handshake completed' in l for l in lines), \
            f"Connection {cid} missing client handshake"
        assert any('[P<>S] SSL handshake completed' in l for l in lines), \
